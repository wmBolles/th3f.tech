...
class TH3FApp {
  ...
  private setupEventListeners(): void {
    ...
    const collaborateBtns = document.querySelectorAll('#collaborate-btn, #footer-collaborate-btn');
    ...
    attackLinks.forEach(link => {
      link.addEventListener('click', (e) => this.handleAttackLink(e));
    });

    collaborateBtns.forEach(btn => {
      btn.addEventListener('click', () => this.openCollaborationDialog());
    });

    closeModalBtn?.addEventListener('click', () => this.hideComingSoonModal());
  }
  ...
  private populateSearchIndex(): void {
    this.searchIndex = [
      { id: 'mitm', title: 'Man in the Middle Attack', category: 'Networking', excerpt: 'Intercepting communications between two parties' },
      { id: 'sql-injection', title: 'SQL Injection', category: 'Web Security', excerpt: 'Database attacks through malicious SQL queries' },
      { id: 'buffer-overflow', title: 'Buffer Overflow', category: 'System Security', excerpt: 'Memory corruption leading to code execution' },
      { id: 'xss', title: 'Cross-Site Scripting', category: 'Web Security', excerpt: 'Client-side code injection attacks' },
      { id: 'dns-spoofing', title: 'DNS Spoofing', category: 'Networking', excerpt: 'Redirecting domain name resolution' },
      { id: 'privilege-escalation', title: 'Privilege Escalation', category: 'System Security', excerpt: 'Gaining higher system privileges' },
      { id: 'virus-intro', title: 'Virus Introduction', category: 'Malware Research', excerpt: 'Understanding computer virus fundamentals' },
      { id: 'c-fundamentals', title: 'C Programming', category: 'Malware Research', excerpt: 'C programming for security research' },
      { id: 'assembly-basics', title: 'Assembly Language', category: 'Malware Research', excerpt: 'Low-level programming for security' },
      { id: 'metamorphic-techniques', title: 'Metamorphic Viruses', category: 'Malware Research', excerpt: 'Advanced virus mutation techniques' },
      { id: 'binary-analysis', title: 'Binary Analysis', category: 'Reverse Engineering', excerpt: 'Analyzing compiled software' },
    ];
  }
  ...
  private handleSearch(query: string): void {
    ...
    const filtered = this.searchIndex.filter(result =>
  ...
  private displaySearchResults(results: SearchItem[]): void {
    const container = document.getElementById('search-results');
    if (!container) return;

    if (results.length === 0) {
      container.innerHTML = '<div class="p-4 text-gray-500 dark:text-gray-400">No results found</div>';
    } else {
      container.innerHTML = results.map(result => `
        <div class="p-4 hover:bg-gray-50 dark:hover:bg-dark-700 cursor-pointer border-b border-gray-100 dark:border-dark-600 last:border-b-0" 
             onclick="app.selectSearchResult('${result.id}')">
          <div class="font-semibold text-gray-900 dark:text-gray-100">${result.title}</div>
          <div class="text-sm text-gray-600 dark:text-gray-400">${result.category}</div>
          <div class="text-sm text-gray-500 dark:text-gray-500 mt-1">${result.excerpt}</div>
        </div>
      `).join('');
    }
  ...
  public selectSearchResult(attackId: string): void {
    this.hideSearchResults();
    this.showAttackContent(attackId);
  ...
  private handleNavigation(e: Event): void {
    ...
    if (section === 'home') {
      ...
      const parentSection = target.closest('.nav-section');
      if (parentSection) {
        document.querySelectorAll('.nav-section').forEach(sec => {
          sec.classList.remove('active');
          const subMenu = sec.querySelector('.sub-menu');
  ...
  private handleAttackLink(e: Event): void {
    e.preventDefault();
    const target = e.target as HTMLElement;
    const attackType = target.getAttribute('data-attack');
    
    if (attackType) {
      this.showAttackContent(attackType);
    }
  }

  private showHomeContent(): void {
    const homeContent = document.getElementById('home-content');
    const attackContent = document.getElementById('attack-content');
    
    homeContent?.classList.remove('hidden');
    homeContent?.classList.add('active');
    attackContent?.classList.add('hidden');
    attackContent?.classList.remove('active');
  }

  private showAttackContent(attackType: string): void {
    const homeContent = document.getElementById('home-content');
    const attackContent = document.getElementById('attack-content');
    const attackDetails = document.getElementById('attack-details');
    
    if (!attackDetails) return;

    homeContent?.classList.add('hidden');
    homeContent?.classList.remove('active');
    attackContent?.classList.remove('hidden');
    attackContent?.classList.add('active');

    attackDetails.innerHTML = this.getAttackContent(attackType);
    attackDetails.scrollIntoView({ behavior: 'smooth' });
  }

  private getAttackContent(attackType: string): string {
...
void secure_function(char *input) {
  ...
  private openCollaborationDialog(): void {
    const message = "Hi! I'm interested in collaborating on TH3F.TECH. I'd love to contribute to the cybersecurity education platform.";
    const whatsappUrl = `https://wa.me/212625218307?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, '_blank');
  }
}

const app = new TH3FApp();
